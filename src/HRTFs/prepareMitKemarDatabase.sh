...
do
	...
	sox -c2 $a -c1 $nearFileL mixer -l
	echo $elevation $reversed MIT_KEMAR/$nearFileL >> ../mitKemarDiffuseL.hrtfs
	if [ "$azimuth" == "0" -o "$azimuth" == "180" ]; then continue; fi # angles which are their symetric
	sox -c2 $a -c1 $farFileL mixer -r
